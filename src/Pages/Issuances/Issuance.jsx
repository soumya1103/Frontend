...
import { addIssuance, getIssuances, updateIssuance } from "../../Api/Service/IssuanceService";
...
function Issuances() {
  ...
  const loadIssuances = async () => {
    try {
      ...
      const issuancesData = response.data.map((issuance) => ({
        ...issuance,
        operation: <Operation widthE="130%" showExtra={false} isBooksPage={false} isIssuance={true} onClickEdit={() => handleEditIcon(issuance)} />,
      }));
  ...
  const handleEditIcon = async (issuance) => {
    const getUser = await getUsersByCredential(issuance.userCredential);
    const getBook = await getBookByTitle(issuance.bookTitle);

    setIssuanceData({
      userId: getUser.data.userId,
      bookId: getBook.data.bookId,
  ...
  return (
    <div className="pages-outer-container">
      ...
      <Modal show={showModal} onClose={handleCloseModal} height="370px" width="400px">
        {isEdit ? <p className="form-title">Edit Issuance</p> : <p className="form-title">Add Issuance</p>}

        <div className="form-content">
          {isEdit ? (
            <>
              <Input label="Phone No." name="userCredential" value={issuanceData?.userCredential} type="text" readOnly={true} />
              <Input label="User Name" name="userName" value={user?.userName} type="text" readOnly={true} />
              <Input label="Book Title" name="bookTitle" value={issuanceData.bookTitle} type="text" readOnly={true} />
            </>
          ) : (
            <>
              ...
              <Input label="Book Author" name="bookAuthor" value={book?.bookAuthor} type="text" readOnly={true} />
            </>
          )}
          <Input label="Return Date" name="returnDate" value={issuanceData.returnDate} type="datetime-local" onChange={(e) => handleChange(e)} />
          {isEdit ? (
            <>
              <label className="form-field-label">Status</label>
              <select name="status" className="form-field-input" value={issuanceData.status} onChange={(e) => handleChange(e)}>
          ...
          {isEdit ? (
            <Input label="Issuance Type" name="issuanceType" value={issuanceData.issuanceType} readOnly={true} />
          ) : (
            <>
              <label className="form-field-label">Issuance Type</label>
              <select name="issuanceType" className="form-field-input" value={issuanceData.issuanceType} onChange={(e) => handleChange(e)}>
        ...
        <div className="form-submit-btn">
          {isEdit ? <Button onClick={onSumbitEditIssuanceHandler}>Update</Button> : <Button onClick={onSumbitAddIssuanceHandler}>Add</Button>}
        </div>
      </Modal>
    </div>
  );
